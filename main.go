...
func (t Training) meanSpeed() float64 {
	...
	if t.Duration.Hours() == 0 {
		fmt.Println("Тренировка не состоялась, продолжительность: 0 часов, 0 минут")
		return 0
	}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	calMSM := float64(CaloriesMeanSpeedMultiplier)
	calMSS := CaloriesMeanSpeedShift
	mIH := float64(MinInHours)

	return (calMSM*r.meanSpeed() + calMSS) * float64(r.Weight) / MInKm * r.Duration.Hours() * mIH
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	calWM := CaloriesWeightMultiplier
	calSHM := CaloriesSpeedHeightMultiplier
	weight := calWM * w.Weight
	heightInM := w.Height / CmInM
	meanSpeed := math.Pow(w.meanSpeed()*KmHInMsec, 2)
	result := (weight + (meanSpeed/heightInM)*calSHM*w.Weight) * w.Duration.Hours() * MinInHours
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	lengthPool := float64(s.LengthPool)
	...
	return lengthPool * countPool / MInKm / duration
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimCalMSS := SwimmingCaloriesMeanSpeedShift
	swimCalWM := float64(SwimmingCaloriesWeightMultiplier)

	return (s.meanSpeed() + swimCalMSS) * swimCalWM * s.Weight * s.Duration.Hours()
}
